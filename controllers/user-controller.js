...
import { deletImage } from '../service/image-del.js'
...
//edit login google
export async function editLoginGooglePatient(req, res) {
    const reqF = req.file
    const userGoogle = await user.findOneAndUpdate({ _id: req.params.id },
        {
...
export async function updateUser(req, res) {
    ...
    const verifUser = await user.updateOne({ _id: req.params.id },
        {
            fullName: req.body.fullName,
            email: req.body.email
        },
    ...
    if (verifUser.modifiedCount === 0) return res.status(400).json({ error: "Unable to update" });
    return res.status(200).json({ verifUser, message: "Success: User Is Updated" });
